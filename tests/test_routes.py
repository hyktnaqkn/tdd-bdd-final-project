...
class TestProductRoutes(TestCase):
    ...
    def test_update_product(self):
        ...
        updated_product["price"] = str(Decimal("99.99"))  # muss String sein für JSON
        updated_product["available"] = not test_product.available
        updated_product["category"] = "TOOLS"

        response = self.client.put(
            f"{BASE_URL}/{test_product.id}", json=updated_product
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertEqual(data["available"], not test_product.available)
    ...
    def test_delete_product(self):
        ...
        # Prüfe, dass es wirklich weg ist
        response = self.client.get(f"{BASE_URL}/{test_product.id}")
        self.assertEqual(response.status_code, status.HTTP_404_NOT_FOUND)
    ...
    def test_query_product_by_availability(self):
        ...
        response = self.client.get(
            f"{BASE_URL}?available={str(target_available).lower()}"
        )
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        ...
        self.assertGreater(len(data), 0)
        for product in data:
            self.assertEqual(product["available"], target_available)
